...
fn total_distance(crabs: &[i64], point: i64) -> i64 {
    crabs
        .iter()
        .fold(0, |total, crab| total + (crab - point).abs())
}

fn magnified_total_distance(crabs: &[i64], point: i64) -> i64 {
...
fn calculate_min_fuel(crabs: &[i64], magnitude: fn(crabs: &[i64], point: i64) -> i64) -> i64 {
    let largest = crabs
        .iter()
